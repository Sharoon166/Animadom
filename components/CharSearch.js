      ...
      <div className="flex justify-center items-center mt-10 relative">
        <div className="relative w-full max-w-md flex items-center">
          <div className="relative flex-grow">
            ...
            <input
              type="text"
              className="w-full pl-10 pr-4 py-2 bg-gray-800 text-white rounded-full border-2 border-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition duration-200 ease-in-out"
              placeholder="Search for a character..."
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <Link
            href={`/character?name=${encodeURIComponent(search)}`}
            className="bg-purple-500 text-white px-4 py-2 rounded-full hover:bg-purple-600 transition duration-200 ease-in-out ml-2"
          >
            Search
          </Link>
        </div>
