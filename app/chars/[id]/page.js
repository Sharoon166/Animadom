...
const CharacterPage = ({ params,data }) => {
  ...
  useEffect(()=>{
    fetch(`https://api.jikan.moe/v4/characters/${params.id}`).then((res)=>res.json()).then((data)=>setCharacterData(data?.data))
    fetch(`https://api.jikan.moe/v4/characters/${params.id}/anime`).then((res)=>res.json()).then((data)=>setAnimeData(data?.data))
